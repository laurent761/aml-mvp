...
import { UsagePanel, UsageBadge, AttackerModeNotice, combineUsage, type UsageSummary } from "./usage-summary";
...
import { ExecutionProblem, TargetReadiness, type ImageReadiness } from "./target-readiness";
...
interface CampaignRow {
  ...
  run_kind: string;
  status: string;
  image_readiness?: ImageReadiness | null;
...
export default function ControlPlatform() {
  ...
  return (
    <div className="control-shell">
        ...
        <main id="main-content" className="content" tabIndex={-1}>
          ...
          {view === "tour" ? null : loading && !data.overview && !data.targets.length && !data.campaigns.length && !data.episodes.length ? <LoadingSurface /> : (
            <>
              ...
              {view === "trajectories" ? <TrajectoriesView onSelectVersion={() => { setTourDraft(null); setCampaignOpen(true); }} data={data} apiBase={apiBase} search={search} selectedId={trajectoryId} onSelect={openTrajectory} onFinding={setSelectedFinding} onChanged={refresh} /> : null}
      ...
      <CampaignSheet onSelectVersion={() => { setSelectedCampaign(null); setTourDraft(null); setCampaignOpen(true); }} key={selectedCampaign?.campaign_id ?? "campaign-closed"} campaign={selectedCampaign} apiBase={apiBase} onOpenChange={(open) => !open && setSelectedCampaign(null)} onChanged={refresh} />
...
function Status({ value }: { value: string }) {
  ...
  return <span className={`status status--${tone}`}><Icon aria-hidden="true" />{normalized === "BLOCKED" ? "Cannot run — target unavailable" : value.replaceAll("_", " ")}</span>;
}
...
export function OverviewView({ data, connected, navigate, onTarget, onCampaign, onSelectCampaign, onSelectFinding }: { data: PlatformData; connected: boolean; navigate: (view: View) => void; onTarget: () => void; onCampaign: () => void; onSelectCampaign: (row: CampaignRow) => void; onSelectFinding: (row: FindingRow) => void }) {
  ...
  return <div className="view-stack overview-workspace">
    ...
    <section className="ledger research-ledger" aria-label="Research totals">
      ...
      <UsagePanel usage={connected ? combineUsage(data.campaigns) : undefined} compact />
    </section>
    <div className="overview-grid">
      <section className="surface"><div className="section-head"><div><h3>Research in motion</h3><p>Campaign objectives, experiment budgets, and observed outcomes.</p></div><Button variant="ghost" size="sm" onClick={() => navigate("campaigns")}>View all<ArrowUpRight /></Button></div>
        {recent.length ? <div className="campaign-cards">{recent.map((campaign) => {
          ...
          return <article className="campaign-card" key={campaign.campaign_id}><div className="campaign-card-top"><span className="target-label"><Target />{targetName(data, campaign.target_version_id)}</span><Status value={campaign.status} /></div><button className="objective-link" onClick={() => onSelectCampaign(campaign)}>{campaignObjective(data, campaign)}<ArrowUpRight /></button><UsageBadge usage={campaign.usage_summary} /><div className="campaign-card-meta"><span>{humanize(campaign.search_mode)} search</span><span>{count} finding{count === 1 ? "" : "s"}</span><code>{shortId(campaign.campaign_id)}</code></div><BudgetMeter label="Experiments started" used={campaign.episodes_started} limit={task?.document.max_episodes} /></article>;
...
function CampaignTable({ rows, onSelect, compact = false, data }: { rows: CampaignRow[]; onSelect: (row: CampaignRow) => void; compact?: boolean; data?: PlatformData }) {
  return <Table><TableHeader><TableRow><TableHead>Campaign</TableHead><TableHead>Mode</TableHead><TableHead>Status</TableHead><TableHead>Experiments</TableHead><TableHead>Usage / pricing</TableHead>{!compact ? <TableHead>Created</TableHead> : null}<TableHead><span className="sr-only">Open</span></TableHead></TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={row.campaign_id}><TableCell><button className="campaign-row-link" onClick={() => onSelect(row)}><strong className="wrap-cell">{data ? campaignObjective(data, row) : shortId(row.campaign_id)}</strong><small className="cell-note">{data ? targetName(data, row.target_version_id) : humanize(row.run_kind)} · {shortId(row.campaign_id)}</small></button></TableCell><TableCell>{row.search_mode}</TableCell><TableCell><Status value={row.status} /></TableCell><TableCell>{row.episodes_started}</TableCell><TableCell><UsagePanel usage={row.usage_summary} compact /></TableCell>{!compact ? <TableCell>{formatDate(row.created_at)}</TableCell> : null}<TableCell><Button variant="ghost" size="icon-sm" onClick={() => onSelect(row)} aria-label={`Open campaign ${shortId(row.campaign_id)}`}><ChevronRight className="row-chevron" /></Button></TableCell></TableRow>)}</TableBody></Table>;
}

function TargetsView({ data, apiBase, search, onTarget, onVersion, onTask, onInspect }: { data: PlatformData; apiBase: string; search: string; onTarget: () => void; onVersion: () => void; onTask: () => void; onInspect: (inspection: Inspection) => void }) {
  ...
  return <div className="view-stack">
    ...
    <section className="surface">
      <div className="section-head"><div><h3>Target registry</h3><p>Registration saves a target version. Readiness checks whether its image is available on the execution host.</p></div><span className="section-meta">{data.versions.length} immutable versions</span></div>
      {targets.length ? <Table><TableHeader><TableRow><TableHead>Target</TableHead><TableHead>Latest registered version</TableHead><TableHead>Versions</TableHead><TableHead>Tasks</TableHead><TableHead>Registered</TableHead></TableRow></TableHeader><TableBody>{targets.map((target) => {
    ...
    <section className="surface">
      ...
      {data.versions.length ? <Table><TableHeader><TableRow><TableHead>Version</TableHead><TableHead>Image readiness</TableHead><TableHead>Image details</TableHead><TableHead>Registered</TableHead><TableHead><span className="sr-only">Inspect</span></TableHead></TableRow></TableHeader><TableBody>{data.versions.filter((row) => JSON.stringify(row).toLowerCase().includes(query)).map((row) => <TableRow key={row.target_version_id}><TableCell><button className="inspector-link" onClick={() => onInspect({ title: `Target version ${shortId(row.target_version_id)}`, description: "Immutable target manifest", path: `/v1/target-versions/${row.target_version_id}` })}><strong className="mono-link">{shortId(row.target_version_id)}</strong></button></TableCell><TableCell><TargetReadiness key={row.target_version_id} apiBase={apiBase} versionId={row.target_version_id} /></TableCell><TableCell><details className="raw-details"><summary>Image reference</summary><code className="image-ref">{row.image}</code></details></TableCell><TableCell>{formatDate(row.created_at)}</TableCell><TableCell><Button variant="ghost" size="icon-sm" onClick={() => onInspect({ title: `Target version ${shortId(row.target_version_id)}`, description: "Immutable target manifest", path: `/v1/target-versions/${row.target_version_id}` })} aria-label={`Inspect target version ${shortId(row.target_version_id)}`}><ChevronRight /></Button></TableCell></TableRow>)}</TableBody></Table> : <EmptyState icon={Boxes} title="No target versions" copy="Attach a digest-pinned OCI manifest to a registered target." action={<Button size="sm" onClick={onVersion}><Plus />Add version</Button>} />}
    </section>
    <section className="surface">
...
function CampaignsView({ data, search, onCreate, onSelect }: { data: PlatformData; search: string; onCreate: () => void; onSelect: (row: CampaignRow) => void }) {
  ...
  return <div className="view-stack">
    ...
    <div className="summary-line"><span><CircleDot />{running} active</span><span><CircleCheck aria-hidden="true" />{campaigns.filter((row) => row.status === "COMPLETED").length} completed</span><UsagePanel usage={combineUsage(campaigns)} compact /></div>
    <section className="surface">
...
export function FindingsView({ data, search, onSelect }: { data: PlatformData; search: string; onSelect: (row: FindingRow) => void }) {
  ...
  return <div className="view-stack"><PageHeading title="Verified exploits" description="Deterministic forbidden-state findings, the trajectories that caused them, and their replay evidence." />
    ...
    <section className="surface"><div className="section-head"><div><h3>Exploit evidence ledger</h3><p>Finding records can share a root cause. Independent replay is shown separately.</p></div><span className="section-meta">{filtered.length} shown</span></div>{filtered.length ? <div className="exploit-ledger">{filtered.map((finding) => { const campaign = data.campaigns.find((row) => row.campaign_id === finding.campaign_id); return <button key={finding.finding_id} onClick={() => onSelect(finding)} className="exploit-row"><span className="finding-icon"><Fingerprint /></span><span className="exploit-main"><small>{campaign ? targetName(data, campaign.target_version_id) : shortId(finding.campaign_id)}</small><strong>{humanize(finding.verifier_id)}</strong><span className="mono">{shortId(finding.episode_id)} · {shortId(finding.finding_id)}</span></span><span className="exploit-proof"><UsageBadge usage={campaign?.usage_summary} /><span className="proof-label proof-label--confirmed"><CircleCheck />Deterministic finding</span><ReproductionLabel finding={finding} data={data} /></span><span className="exploit-severity"><strong>{finding.severity.toFixed(2)}</strong><small>Configured severity</small></span><Status value="VERIFIED" /><ArrowUpRight /></button>; })}</div> : <EmptyState icon={Fingerprint} title="No matching exploit findings" copy={data.findings.length ? "Adjust the status or search filter." : "A finding appears when a deterministic verifier confirms that an experiment reached a forbidden state."} />}</section>
  </div>;
}
...
export function LearningView({ data, search, onInspect, onExperiments }: { data: PlatformData; search: string; onInspect: (inspection: Inspection) => void; onExperiments: () => void }) {
  ...
  return <div className="view-stack"><PageHeading title="Learning" description="Inspect reusable strategies, observed outcomes, and mutations that inform the next search." action={<Button variant="outline" onClick={onExperiments}><FlaskConical />Compare experiments<ArrowUpRight /></Button>} />
    <div className="panel-footnote"><CircleAlert /><span>Strategy memory can include simulated runs. Historical strategy records do not identify every contributing call. Inspect campaign usage before treating these outcomes as real model evidence.</span></div>
...
function TrajectoriesView({ data, apiBase, search, selectedId, onSelect, onFinding, onChanged, onSelectVersion }: { data: PlatformData; apiBase: string; search: string; selectedId: string; onSelect: (id: string) => void; onFinding: (row: FindingRow) => void; onChanged: () => void; onSelectVersion: () => void }) {
  const [campaignId, setCampaignId] = useState("");
  ...
  return <div className="view-stack"><PageHeading title="Trajectories" description="Reconstruct each experiment: attacker action, target observation, reward, and trusted consequence." /><div className="lab-toolbar"><div className="workbench-select"><Label htmlFor="trajectory-campaign">Campaign</Label><NativeSelect id="trajectory-campaign" value={campaignId} onChange={(event) => setCampaignId(event.target.value)}><NativeSelectOption value="">All loaded campaigns</NativeSelectOption>{data.campaigns.map((row) => <NativeSelectOption key={row.campaign_id} value={row.campaign_id}>{targetName(data, row.target_version_id)} · {shortId(row.campaign_id)}</NativeSelectOption>)}</NativeSelect></div><div className="workbench-select"><Label htmlFor="trajectory-outcome">Experiment outcome</Label><NativeSelect id="trajectory-outcome" value={outcome} onChange={(event) => setOutcome(event.target.value)}><NativeSelectOption value="all">All outcomes</NativeSelectOption><NativeSelectOption value="success">Forbidden state reached</NativeSelectOption><NativeSelectOption value="no_success">No forbidden state observed</NativeSelectOption><NativeSelectOption value="error">Execution interrupted</NativeSelectOption><NativeSelectOption value="unfinished">In progress</NativeSelectOption></NativeSelect></div><span className="quiet-label">{filtered.length} of {data.episodes.length} loaded experiments</span></div>
    ...
    <section className="surface trajectory-canvas">{!selectedId ? <EmptyState icon={GitBranch} title="Follow the attack, step by step" copy="Select an experiment to explore the complete path and its observed outcome." /> : <><div className="section-head"><div><h3>{shortId(selectedId)}</h3><p>{sourceCampaign ? `${targetName(data, sourceCampaign.target_version_id)} · ${shortId(sourceCampaign.campaign_id)}` : "Recorded experiment"}</p></div><Button variant="outline" size="sm" disabled={busy || !detail.data} onClick={exportEvidence}><ArrowDownToLine />{busy ? "Exporting" : "Export evidence"}</Button></div>{detail.error ? <div className="inline-failure" role="alert"><p>{detail.error}</p><Button variant="outline" size="sm" onClick={detail.retry}>Retry</Button></div> : null}{!detail.data && !detail.error ? <LoadingSurface /> : null}{detail.data ? <><div className="trajectory-context"><Outcome episode={detail.data} /><span>Seed {detail.data.seed}</span><span>Reward <strong>{detail.data.cumulative_reward.toFixed(3)}</strong></span><span>Lifecycle: {humanize(detail.data.status)}</span></div>{detail.data.error ? <ExecutionProblem error={detail.data.error} noActions={detail.data.steps.length === 0} apiBase={apiBase} versionId={sourceCampaign?.target_version_id} onSelectVersion={onSelectVersion} /> : null}<TrajectoryExplorer key={selectedId} episode={detail.data} />{findings.length ? <div className="trajectory-findings"><span className="eyebrow">VERIFIED FINDINGS FROM THIS EXPERIMENT</span>{findings.map((row) => <button key={row.finding_id} onClick={() => onFinding(row)}><Fingerprint /><span>{humanize(row.verifier_id)}</span><ArrowUpRight /></button>)}</div> : null}</> : null}</>}</section></div>
  </div>;
}
export function TrajectoryExplorer({ episode, compact = false }: { episode: EpisodeDetail; compact?: boolean }) {
  ...
  if (!steps.length) return <EmptyState icon={GitBranch} title={episode.error ? "No attacks ran" : "No actions recorded yet"} copy={episode.error ? "Execution stopped before the first action. This experiment has no security result." : "An experiment's trajectory appears after the attacker takes its first action."} />;
  return <div className={compact ? "trajectory-explorer is-compact" : "trajectory-explorer"}>
    <UsagePanel usage={episode.usage_summary} />
...
function CampaignDialog({ open, apiBase, data, initialSelection, onOpenChange, onSuccess }: { open: boolean; apiBase: string; data: PlatformData; initialSelection: TourSelection | null; onOpenChange: (open: boolean) => void; onSuccess: (campaign: CampaignRow) => void }) {
  ...
  const handleReady = useCallback((id: string, ready: boolean) => setReadyVersion(ready ? id : ""), []);
  ...
  const submit = async () => {
    if (busy || readyVersion !== selectedVersionId || !budgetAcknowledged) return;
    setBusy(true);
    try {
  ...
  return <Dialog open={open} onOpenChange={close}><DialogContent className="dialog-wide"><DialogHeader><DialogTitle>Start campaign</DialogTitle><DialogDescription>The orchestrator enforces task budgets and gives every branch a fresh sealed episode.</DialogDescription></DialogHeader><div className="form-stack"><Label htmlFor="campaign-version">Target version</Label><NativeSelect id="campaign-version" className="w-full" value={selectedVersionId} onChange={(event) => { setVersionId(event.target.value); setReadyVersion(""); setTaskId(""); setBudgetAcknowledged(false); }}><NativeSelectOption value="">Select a version</NativeSelectOption>{data.versions.map((version) => <NativeSelectOption key={version.target_version_id} value={version.target_version_id}>{targetName(data, version.target_version_id)} · {shortId(version.target_version_id)} · {formatDate(version.created_at)}</NativeSelectOption>)}</NativeSelect>{open && selectedVersionId ? <TargetReadiness key={selectedVersionId} apiBase={apiBase} versionId={selectedVersionId} autoPrepare onReady={handleReady} /> : null}<Label htmlFor="campaign-task">Attack task</Label><NativeSelect id="campaign-task" className="w-full" value={selectedTaskId} onChange={(event) => { setTaskId(event.target.value); setBudgetAcknowledged(false); }}><NativeSelectOption value="">Select a task</NativeSelectOption>{tasks.map((task) => <NativeSelectOption key={task.attack_task_id} value={task.attack_task_id}>{shortId(task.attack_task_id)} · {String(task.document.objective ?? "Untitled objective")}</NativeSelectOption>)}</NativeSelect><div className="form-grid"><div><Label htmlFor="campaign-mode">Search mode</Label><NativeSelect id="campaign-mode" className="w-full" value={mode} onChange={(event) => setMode(event.target.value)}><NativeSelectOption value="adaptive">Adaptive beam</NativeSelectOption><NativeSelectOption value="linear">Linear baseline</NativeSelectOption></NativeSelect></div></div><Label htmlFor="campaign-red">Red experiment config</Label><NativeSelect id="campaign-red" className="w-full" value={configId} onChange={(event) => setConfigId(event.target.value)}><NativeSelectOption value="">Backend default</NativeSelectOption>{data.redConfigs.map((config) => <NativeSelectOption key={config.red_config_id} value={config.red_config_id}>{config.name}</NativeSelectOption>)}</NativeSelect><AttackerModeNotice provider={typeof asObject(data.redConfigs.find(row => row.red_config_id === configId)?.document.model).provider === "string" ? String(asObject(data.redConfigs.find(row => row.red_config_id === configId)?.document.model).provider) : undefined} />{budget ? <section className="budget-confirm" aria-label="Campaign budget envelope"><div><span>Episodes</span><strong>{String(budget.max_episodes ?? "—")}</strong></div><div><span>Steps / episode</span><strong>{String(budget.max_steps_per_episode ?? "—")}</strong></div><div><span>Token budget units</span><strong>{formatNumber(Number(budget.max_model_tokens ?? 0))}</strong></div><div><span>Cost ceiling</span><strong>{formatCost(Number(budget.max_total_cost ?? 0))}</strong></div><div><span>Wall time</span><strong>{String(budget.max_wall_time_seconds ?? "—")}s</strong></div><div><span>Concurrency</span><strong>{String(budget.max_concurrency ?? "—")}</strong></div></section> : null}<label className="budget-ack"><Checkbox checked={budgetAcknowledged} onCheckedChange={(value) => setBudgetAcknowledged(value === true)} /><span>I reviewed the complete task budget and want to queue this campaign.</span></label></div><DialogFooter><Button variant="outline" onClick={() => close(false)}>Cancel</Button><Button onClick={submit} disabled={busy || readyVersion !== selectedVersionId || !selectedVersionId || !selectedTaskId || !budgetAcknowledged}>{busy ? <RefreshCw className="spin" /> : <Play />}{busy ? "Queuing" : "Queue campaign"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
interface CampaignMetrics { usage_summary?: UsageSummary | null; episodes_started: number; episodes_recorded: number; successful_episodes: number; step_count: number; effect_count: number; finding_count: number; tokens_used: number; cost_used: number; }
interface EpisodeDetail extends EpisodeRow { usage_summary?: UsageSummary | null; steps: JsonObject[]; effects: JsonObject[]; verifier_events: JsonObject[]; model_invocations: JsonObject[]; }

function CampaignSheet({ campaign, apiBase, onOpenChange, onChanged, onSelectVersion }: { campaign: CampaignRow | null; apiBase: string; onOpenChange: (open: boolean) => void; onChanged: () => void; onSelectVersion: () => void }) {
